...
class TestKnowledgeStore:
    ...
    def test_query_exact_found(self, store):
        """精确查询返回正确概念。"""
    ...
    def test_query_exact_not_found(self, store):
        """不存在的概念返回 None。"""
        result = store.query_exact("accounting", "NonExistent")
        assert result is None

    def test_query_exact_wrong_domain(self, store):
        """错误的 domain 返回 None。"""
    ...
    def test_list_empty_domain(self, store):
        """无概念的领域返回空列表。"""
        concepts = store.list_by_domain("medical")
        assert concepts == []

    def test_source_level_range(self, store):
    ...
    def test_count_accurate(self, store):
        """count() 返回准确数。"""
        assert store.count() == len([
            c for c in store.list_by_domain("accounting")
        ])
