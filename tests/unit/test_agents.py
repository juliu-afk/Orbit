...
def test_factory_accepts_string():
    """工厂接受字符串角色名。"""
...
@pytest.mark.asyncio
async def test_developer_mock_output():
    """无 LLM 时返回 mock 结果。"""
...
@pytest.mark.asyncio
async def test_architect_mock_output():
...
@pytest.mark.asyncio
async def test_reviewer_mock_output():
    agent = ReviewerAgent()
...
@pytest.mark.asyncio
async def test_config_manager_mock_output():
