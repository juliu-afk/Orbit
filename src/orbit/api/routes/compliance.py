...
@router.get("/validate", summary="验证知识概念合规性")
async def validate_concept(
    ...
    if result is None:
        raise HTTPException(status_code=404, detail=f"概念 {domain}/{concept} 不存在")
    return result.to_dict()


@router.get("/validate-all", summary="批量验证领域合规性")
...
@router.get("/rules", summary="列出合规规则")
