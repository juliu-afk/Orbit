...
[← 返回目录 || Back to index](README.md) · [← 上一章 || Prev: API 参考 || API Reference](06-api-reference.md)
...
测试决策树 + 9 项门禁（见 §7.3）。全绿方可 PR 合入。 || Test decision tree + 9 gates (see §7.3). All green before PR merge.

## 7.2 模块地图速查 || Module Map

完整职责表见 [02 整体架构 §2.3](02-architecture.md#23-模块地图--module-map)。核心模块改动触发强制回归： || See [Architecture §2.3](02-architecture.md#23-模块地图--module-map) for the full responsibility table. Core module changes trigger mandatory regression:
...
|---|---|---|---|---|---|---|---|---|---|
| 单元 || Unit | `tests/unit/` | ~255 | `pytest tests/unit/ -q` | 秒级 || Seconds |
| 集成 || Integration | `tests/integration/` | 12 | `pytest tests/integration/ -q` | ≤1min |
...
[← 返回目录 || Back to index](README.md) · [下一章：附录 → || Next: Appendix →](08-appendix.md)
