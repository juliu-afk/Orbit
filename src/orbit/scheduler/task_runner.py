...
class TaskRunner:
    ...
    async def _agent_cycle(self, task_id: str, state: TaskState, context: dict[str, Any]) -> str:
        ...
        if role and self._agent_factory is not None:
            ...
            # WHY 意图路由: 聊天应结束任务，编程意图才进入 PARSING/clarifier
            if state == TaskState.IDLE and role == "chatter":
                ...
                if intent == "programming":
                    logger.info("chatter_intent_programming", task_id=task_id)
                else:
                    logger.info("chatter_intent_chat", task_id=task_id)
    ...
    def _extract_chatter_intent(output: str) -> str:
        ...
        ChatAgent 返回结构化 JSON，其中含 __intent__ 字段。
        此方法从 _run_agent 的字符串输出中恢复意图。
        """
