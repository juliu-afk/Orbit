...
|---|---|---|
| `/health` | `health.py` | `GET /health`（含 Redis 探针）|| includes Redis probe |
| `/api/v1/tasks` | `tasks.py` | `POST /tasks` 创建 || create · `GET /tasks/{id}` · `POST /tasks/{id}/cancel` |
| `/api/v1/goal` | `goal.py` | `POST /goal`（PRD/模糊需求/批量目录）|| (PRD/ambiguous requirements/batch directories)· `POST /goal/{id}/cancel · pause · resume` |
| `/api/v1/loop` | `loop.py` | `POST /loop` 定时循环 || scheduled loop · `GET /loop` · `DELETE /loop/{id}` |
| `/api/v1/compose` | `compose.py` | `POST /compose/run` 执行 spec 编排（限流 5 req/60s）|| execute spec orchestration (rate-limited 5 req/60s) |
...
| 前缀 || Prefix | 路由文件 || Route file | 代表端点 || Representative endpoints |
|---|---|---|
| `/api/v1/review` | `review.py` | `POST /review` 创建审查 || create review · `GET /review/{id}` |
...
| 前缀 || Prefix | 路由文件 || Route file | 代表端点 || Representative endpoints |
|---|---|---|
| `/api/v1/files` | `files_routes.py` | `GET /tree` 文件树 || file tree · `GET /read?path=...` |
| `/api/v1/git` | `git_routes.py` | Git 操作 || Git operations（commit/branch/diff/log）|
| `/api/v1/search` | `search_routes.py` | `GET /search?q=...` 文件名 + 内容搜索 || filename + content search（ripgrep）|
| `/api/v1/codegraph` | `codegraph_routes.py` | `GET /definition` Go-to-Def · `/references` · `/outline` · `/hover` · 图谱可视化 || graph visualization · `POST /build` |
| `/api/v1/tests` | `tests_routes.py` | 测试结果 + 覆盖率 API || Test results + coverage API |
| `/api/v1/insights` | `insights_routes.py` | 风险评分、影响分析、模块健康 || Risk scoring, impact analysis, module health |
...
|---|---|---|
| WebSocket | `/ws` | 驾驶舱实时监控（DAG/Token/告警推送）|| Cockpit real-time monitoring (DAG/Token/alert push) |
| WebSocket | `/ws/diagnostics/{task_id}` | 实时诊断推送（mypy 结果）|| Real-time diagnostic push (mypy results) |
| WebSocket | `/api/v1/chat` | 自然语言对话 || Natural language conversation |
| SSE | `stream/` 模块 || `stream/` module | Agent 输出流式响应 || Agent output streaming |
