...
| Step 7.7 | 高峰避让延迟调度 | Phase 7（第13周） | 高峰时段Token成本高 | 3-5天 |

> **Step 7.7 说明**: 系统内置厂商高峰时段表，用户批量提交任务时可指定 `defer_to_offpeak=true`。
> OffPeakScheduler 自动将任务排队到低峰窗口执行，低峰窗口满则顺延。详见
> `docs/requirements/2026-07-01-高峰避让调度/阶段1-PRD-高峰避让调度.md`
