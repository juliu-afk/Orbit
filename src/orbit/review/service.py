...
class ReviewService:
    ...
    async def create_review(self, task_id: str, created_by: str) -> Review:
        async with self.session_factory() as session:
            existing = await session.execute(select(Review).where(
                Review.task_id == task_id, Review.status.in_(["pending","in_review","changes_requested"])))
            if existing.scalar_one_or_none(): raise ValueError(f"Task {task_id} has active review")
    ...
    async def get_review(self, review_id: str) -> Review | None:
        async with self.session_factory() as session: return await session.get(Review, review_id)
    ...
    async def record_decision(self, review_id: str, file_path: str, hunk_index: int,
        ...
        async with self.session_factory() as session:
            ...
            await session.commit(); return rd
    ...
    async def transition_status(self, review_id: str, new_status: str) -> Review:
        ...
        async with self.session_factory() as session:
            ...
            await session.commit(); return review
    ...
    async def get_summary(self, review_id: str) -> dict:
        async with self.session_factory() as session:
            ...
            return {"total_files":len(by_file),"files":by_file,"total_decisions":sum(v for f in by_file.values() for v in f.values())}
