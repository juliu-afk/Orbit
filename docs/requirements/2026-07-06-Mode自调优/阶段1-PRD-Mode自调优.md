...
**US3: 模式可见性指示器**
> 作为 Orbit 用户，我希望 Agent 回复时能看到当前使用的模式和技能，
> 以便我知道它正在用什么策略和我交互，而不是黑盒。

验收标准：
- Agent 首条 reply 自动带模式前缀（如 `[🔍 clarify·深度模式]`）
- 切换 mode 后前缀立即变化（如 `/mode fast` → `[🔍 clarify·快速模式]`）
- compose 技能也带前缀（如 `[📋 compose:plan]` 或 `[🧪 compose:tdd]`）
- mode=None（降级默认）时显示 `[🔍 clarify·默认]`
...
| AC8 | Agent 首条 reply 带模式前缀 `[🔍 mode·策略]` | US3 |
| AC9 | compose 技能也带前缀 `[📋 compose:plan]` 等 | US3 |
| AC10 | mode 切换后前缀立即变化（回复可见） | US3 |
| AC11 | 现有测试全绿 | 回归 |
| AC12 | 新增 scorer + mode_tuner + indicator 单元测试 ≥12 条 | 覆盖率 |
