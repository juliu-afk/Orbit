...
class TestMetaGraph:
    ...
    def test_add_and_impact_analysis(self) -> None:
        ...
        try:
            mg.add_relation("code", "PaymentService.process", RelationType.WRITES_TO,
                            "db", "payments")
            mg.add_relation("code", "PaymentService.process", RelationType.READS_FROM,
                            "db", "accounts")
            mg.add_relation("code", "PaymentService.process", RelationType.DEPENDS_ON,
    ...
    def test_config_impact_trace(self) -> None:
        mg = MetaGraph()
        try:
            mg.add_relation("config", "PAYMENT_TIMEOUT", RelationType.DECIDED_AT,
                            "reasoning", "task-001")
            mg.add_relation("config", "PAYMENT_TIMEOUT", RelationType.CONNECTS_TO,
                            "code", "gateway.py")
    ...
    def test_architecture_health_detects_violation(self) -> None:
        ...
        try:
            # 写入数据库但无合规关联 → 腐化
            mg.add_relation("code", "badFunc", RelationType.WRITES_TO,
                            "db", "orders")
    ...
    def test_architecture_health_clean(self) -> None:
        mg = MetaGraph()
        try:
            mg.add_relation("code", "goodFunc", RelationType.WRITES_TO,
                            "db", "orders")
            mg.add_relation("code", "goodFunc", RelationType.COMPLIES_WITH,
                            "knowledge", "accounting_rules")
            violations = mg.architecture_health_check()
            # 有合规关联 → 无违规
