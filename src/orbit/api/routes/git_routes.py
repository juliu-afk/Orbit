...
class GpgKey(BaseModel):
    id: str; name: str; email: str; fingerprint: str

class CommitRequest(BaseModel):
    message: str = Field(..., min_length=1, max_length=500)
    files: list[str] = Field(default_factory=list)
    sign: bool = False; gpg_key_id: str | None = None

@router.get("/gpg-keys", response_model=list[GpgKey])
async def list_gpg_keys():
    try:
        proc = await asyncio.create_subprocess_exec("gpg","--list-secret-keys","--keyid-format","LONG","--with-colons", stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE)
...
async def commit(req: CommitRequest):
    ...
    if req.files: cmd.extend(req.files)
    proc = await asyncio.create_subprocess_exec(*cmd, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE)
    stdout, stderr = await proc.communicate()
    out, err = stdout.decode("utf-8", errors="replace"), stderr.decode("utf-8", errors="replace")
    if proc.returncode != 0: raise HTTPException(status_code=400, detail=f"Commit failed: {err}")
    m = re.search(r"\[[\w\-]+ ([a-f0-9]+)\]", out)
    commit_hash = m.group(1) if m else ""
    verified = False
    if req.sign:
        sp = await asyncio.create_subprocess_exec("git","-C",str(ws),"log","-1","--show-signature", stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE)
