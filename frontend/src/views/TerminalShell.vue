...
function buildTree(files: Array<{ path: string }>): FileNode[] {
  ...
  for (const f of files) {
    ...
    for (let i = 0; i < parts.length; i++) {
      const part = parts[i]; cp = cp ? `${cp}/${part}` : part
...
// v0.23: CSS grid-areas 动态拼接
function gridAreas(): string {
  const cols = settings.fileTreeLeft
    ? 'filetree chat right'
    : settings.agentRight ? 'chat right filetree' : 'right chat filetree'
  return `"${cols}" "statusbar statusbar statusbar"`
}
...
// v0.23: 拖拽分割线
function startResize(edge: 'left' | 'right', e: PointerEvent) {
  e.preventDefault(); (e.target as HTMLElement).setPointerCapture(e.pointerId)
  const onMove = (ev: PointerEvent) => {
    if (edge === 'left') { settings.fileTreeWidth = Math.max(160, Math.min(480, ev.clientX)) }
...
onMounted(async () => {
  ...
  ws.setMessageHandler((msg) => { switch (msg.type) { case 'task:update': task.handleTaskUpdate(msg.payload as Record<string, unknown>); break; case 'metrics:snapshot': agentops.handleWsEvent('metrics:snapshot', msg.payload as Record<string, unknown>); break; case 'agentops:alert': agentops.handleWsEvent('agentops:alert', msg.payload as Record<string, unknown>); break } })
...
<div class="terminal-shell glass" :data-filetree-collapsed="!shell.showFileTree" :style="{ gridTemplateAreas: gridAreas() }" @contextmenu.prevent>
  ...
  <aside class="panel-right" style="border-left:1px solid var(--color-orbit-border);overflow-y:auto">
    <MonacoPanel v-if="shell.showMonaco" /><AgentInfoPanel v-else />
  </aside>
...
<style scoped>
.terminal-shell { display:grid; grid-template-rows:1fr var(--spacing-statusbar); grid-template-areas:v-bind(gridAreas()); height:100vh; overflow:hidden }
