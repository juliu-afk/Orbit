...
def _svc() -> FileService:
    if _file_service is None: raise RuntimeError("FileService not initialized")
...
@router.get("/diff")
async def diff_file(path: str = Query(..., min_length=1),
    rev_a: str = Query("HEAD"), rev_b: str | None = Query(None)):
    try: result = await _svc().diff(path, rev_a, rev_b)
    except ValueError as e: raise HTTPException(status_code=403, detail=str(e))
    return result
