...
核心认知：**纯静态分析拦截幻觉的理论上限只有 48.5%–77%**，必须与动态验证（沙箱执行、运行时追踪）结合，才能把错误率压到 <3%。这直接决定了 9 层防幻觉"从快到慢、静动结合"的分层结构（见 [03 防幻觉](03-hallucination-defense.md)）。 || Key insight: **the theoretical upper bound of pure static analysis for hallucination interception is only 48.5%–77%**; it must be combined with dynamic validation (sandbox execution, runtime tracing) to drive the error rate below 3%. This directly mandates the 9-layer hallucination defense's "fast-to-slow, static+dynamic" layered structure (see [03 Hallucination Defense](03-hallucination-defense.md)).
...
[← 返回目录 || Back to index](README.md) · [下一章：整体架构 → || Next: Architecture →](02-architecture.md)
