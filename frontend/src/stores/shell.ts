...
export const useShellStore = defineStore('shell', () => {
  ...
  const showChart = ref(false); const showSearch = ref(false); const showSchedule = ref(false); const selectedFile = ref<string | null>(null);
  ...
  function closeAllDrawers() { showDAG.value = false; showChart.value = false; showSearch.value = false; showSchedule.value = false }
  return { showFileTree, showMonaco, showDAG, showChart, showSearch, showSchedule, selectedFile, quoteTarget, toggleFileTree, openFileReview, closeFileReview, setQuoteTarget, toggleDAG, toggleChart, toggleSearch, toggleSchedule, closeAllDrawers }
})
