...
[← 返回目录 || Back to index](README.md) · [← 上一章：能力与边界 || Prev: Capabilities & Boundaries](02-capabilities-boundaries.md)
...
## 3.1 三层架构规范 || The Three-Layer Model
...
## 3.2 任务数据流全链路 || End-to-End Data Flow
...
IntakeRouter 意图判定 ──▶ ClarifierAgent 多轮澄清 ──▶ 结构化 PRD
   ▼
Scheduler 状态机  IDLE → PARSING → SCOPING → PLANNING → CODING → VERIFYING → DONE
...
## 3.4 五图谱体系 || The Five-Graph System
...
`READS_FROM` · `WRITES_TO` · `DELETES_FROM`（代码→数据库） · `DEPENDS_ON`（代码→配置/代码） · `OVERRIDES`（配置→配置） · `REFERENCES` · `COMPLIES_WITH`（代码→知识） · `PRODUCED_BY` · `MODIFIED_BY`（代码↔推理链） · `DECIDED_AT`（配置→推理链） · `CONNECTS_TO` · `USED_IN`（知识→代码） || `READS_FROM` · `WRITES_TO` · `DELETES_FROM` (code → database) · `DEPENDS_ON` (code → config/code) · `OVERRIDES` (config → config) · `REFERENCES` · `COMPLIES_WITH` (code → knowledge) · `PRODUCED_BY` · `MODIFIED_BY` (code ↔ reasoning chain) · `DECIDED_AT` (config → reasoning chain) · `CONNECTS_TO` · `USED_IN` (knowledge → code)
...
## 3.5 熔断 / 检查点 / 审计链 || Breaker / Checkpoint / Audit

### 3.5.1 熔断 || Circuit Breaker
...
### 3.5.2 检查点 || Checkpoint（[`src/orbit/checkpoint/manager.py`](../../src/orbit/checkpoint/manager.py)）
...
- **保存时机**：每个关键状态转换点（IDLE→PARSING→SCOPING→PLANNING→CODING→VERIFYING…）。 || **Save timing**: at each key state transition (IDLE→PARSING→SCOPING→PLANNING→CODING→VERIFYING…).
- **RTO 目标**：≤30 秒。 || **RTO target**: ≤30 seconds.

### 3.5.3 审计链 || Audit Trail
...
[← 返回目录 || Back to index](README.md) · [下一章：端到端流程 → || Next: End-to-End Workflow →](04-workflow.md)
