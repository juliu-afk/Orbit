...
async def test_e2e_resume_from_checkpoint(e2e_app):
    """任务执行 → 模拟中断 → 从检查点恢复。
...
@pytest.mark.e2e
@pytest.mark.asyncio(loop_scope="function")
async def test_e2e_mock_llm_fail_count(mock_llm: MockLLMClient):
    """MockLLMClient fail_count 行为正确——前 N 次抛异常。"""
    mock_llm.fail_count = 3

    from orbit.gateway.schemas import LLMRequest
    from tests.e2e.mock_llm import LLMError

    # 前 3 次抛异常
    for i in range(3):
        with pytest.raises(LLMError):
            await mock_llm.generate(LLMRequest(prompt="test"), task_id="t1")
        assert mock_llm.call_count == i + 1

    # 第 4 次成功
    resp = await mock_llm.generate(LLMRequest(prompt="test"), task_id="t1")
