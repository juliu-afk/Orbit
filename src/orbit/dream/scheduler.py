...
class DreamScheduler:
    def __init__(
        self, engine=None, interval_seconds=DREAM_INTERVAL_SECONDS, last_dream_file=LAST_DREAM_FILE
    ):
        self._engine = engine
        self._interval = interval_seconds
        self._last_dream_file = last_dream_file
    ...
    async def check_and_run(self):
        if not self.should_run():
            return DreamResult(status=DreamStatus.SKIPPED, notes=["间隔未到"])
        if self._engine is None:
            return DreamResult(status=DreamStatus.SKIPPED, notes=["引擎未注入"])
