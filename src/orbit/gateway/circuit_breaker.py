...
class CircuitBreaker:
    ...
    # ---- Redis / 内存双层（P2-5: SQLite 持久化兜底）----

    @classmethod
    def _get_state_db(cls) -> str | None:
        """P2-5: 获取 SQLite 持久化路径——不可写时返回 None 降级内存."""
        cache_dir = Path.home() / ".orbit" / "cache"
        try:
            cache_dir.mkdir(parents=True, exist_ok=True)
            db_path = str(cache_dir / "circuit_breaker.db")
            with sqlite3.connect(db_path) as conn:
                conn.execute(
                    ...
                    "  opened_at REAL,"
                    "  half_open INTEGER DEFAULT 0,"
                    "  probe_in_flight INTEGER DEFAULT 0"
                    ")"
                )
                conn.commit()
            return db_path
    ...
    async def _get_state(self, key: str) -> CircuitBreakerState:
        # P2-5: SQLite 持久化优先，不可用降级内存
        db_path = self._get_state_db()
        if db_path:
            try:
                with sqlite3.connect(db_path) as conn:
                    row = conn.execute(
                        "SELECT failure_count, opened_at, half_open, probe_in_flight"
                        " FROM cb_state WHERE key=?",
                        (key,),
                    ).fetchone()
                    if row:
                        state = CircuitBreakerState()
                        state.failure_count = row[0]
                        state.opened_at = row[1]
                        state.half_open = bool(row[2])
                        state.probe_in_flight = bool(row[3])
                        return state
            ...
            try:
                with sqlite3.connect(db_path) as conn:
                    conn.execute(
                        ...
                        " (key, failure_count, opened_at, half_open, probe_in_flight)"
                        " VALUES (?, ?, ?, ?, ?)",
                        (
                            key,
                            state.failure_count,
